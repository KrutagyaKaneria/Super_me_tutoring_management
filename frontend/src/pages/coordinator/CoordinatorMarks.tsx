...
export function CoordinatorMarks() {
  ...
  const fetchMarks = async () => {
    try {
      setLoading(true);
      const res = await api.get('/coordinator/marks');
      const mapped = (res.data.marks || []).map((m: any) => ({
        id: m._id,
        student: m.studentId?.name || 'Unknown',
        exam: m.examName,
        subject: m.subject,
        marks: m.marks,
        total: m.totalMarks,
  ...
  const handleAddMarks = async () => {
    ...
    try {
      await api.post('/coordinator/add-marks', {
        studentId,
        subject,
        examName,
        date,
  ...
  return (
    <div>
      <PageHeader title="Exam Marks Entry">
        <button
      ...
      <CardShell title="Update Student Marks">
        {loading ? (
            ...
            <div className="space-y-4">
              <div>
                ...
                >
                  ...
                  {students.map(s => <option key={s._id} value={s.user?._id}>{s.user?.name}</option>)}
                </select>
              </div>
